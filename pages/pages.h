...
#define pages_category_len 16 
#define pages_line_len 336
...
#define pages_category_offset(i) pages_note_offset(i) + pages_note_len + 1
#define pages_line_offset(i) pages_category_offset(i) + pages_category_len + 1

#define pages_columns_check(index) \
	(columns.size > index) && (cols+index != NULL) && \
	(cols[index].size > 2) && (strlen(cols[index].text) > 2)
...
__attribute_warn_unused_result__
vector pages_make2(const char *const filename, int mode);
